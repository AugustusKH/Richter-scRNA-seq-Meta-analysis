...
## Load a config file and objects provided from this codes
...
```{r}
# Load the objected generated from this codes (load them as the prolong running for generating)
load.pigz("cds_list.rda")
load.pigz("doubletfinder_list.rda")
load.pigz("cds_main.rda")
...
```{r, eval = FALSE}
# Save file
save.pigz(cds_list, file = "cds_list.rda")
```
...
save.pigz(doubletfinder_list, file = "doubletfinder_list.rda")
```
...
```{r, eval = FALSE}
# Save the cds_main
save.pigz(cds_main, file = "cds_main.rda")
```
...
bb_var_umap(cds_main_filtered, 
            var = "seurat_celltype_l1") + 
  labs(y= "UMAP 2", x = "UMAP 1", title = "Cell annotation")
```
