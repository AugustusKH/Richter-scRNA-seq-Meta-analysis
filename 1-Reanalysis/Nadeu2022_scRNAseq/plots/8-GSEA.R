...
# Load data
path_to_gsea <- here::here("reanalysis/results/4_differential_expression/tmp/patient_specific_gsea_rt_vs_cll.rds")
path_to_gsea_raw <- here::here("reanalysis/results/4_differential_expression/tmp/patient_specific_gsea_raw_rt_vs_cll.rds")
...
# Save
ggsave(
  filename = here::here("reanalysis/results/plots/08.1-rt_gsea_enrichment_plots.pdf"),
  plot = fig_gsea,
  device = cairo_pdf,
  width = 20, 
  height = 13.5,
...
pdf(
  file = here::here("reanalysis/results/plots/08.2-rt_gsea_upregulated_upset_plots_supplementary.pdf"),
  onefile = FALSE,
  width = 3.94,
  height = 3
)
...
dev.off()
pdf(
  file = here::here("reanalysis/results/plots/08.3-rt_gsea_downregulated_upset_plots_supplementary.pdf"),
  onefile = FALSE,
  width = 3.94,
  height = 3
)
...
nes_q_dfs <- purrr::map(gsea_sorted, function(df) {
  df_sub <- df %>%
    ...
    dplyr::select("ID", "Description", "NES", "qvalue")
  df_sub
})
nes_q_df <- bind_rows(nes_q_dfs, .id = "case")
write_delim(
  nes_q_df,
  file = here::here("reanalysis/results/plots/08-rt_gsea_nes_and_qvalues.csv"),
  delim = ";"
)
